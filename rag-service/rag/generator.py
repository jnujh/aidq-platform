...
## 세분화 정보 (groupBreakdown — 있을 때만)
진단 결과에 `groupBreakdown` 필드가 있으면, 어느 그룹에서 품질이 낮은지 특정할 수 있다.
- `unit`: "class"면 그룹이 클래스(예: 사막/바다/숲), "column"이면 컬럼(예: 소득/나이).
- `metrics`: 지표별 "그룹→점수" 맵. 점수 오름차순(나쁜 그룹이 앞). 0~1 스케일.
- `counts`: 그룹별 표본 수(클래스 균형 근거 — 예: 바다 120 vs 사막 400).
활용: 개선 필요 항목을 쓸 때, 그 지표의 groupBreakdown을 보고 **특정 그룹이 두드러지게 낮으면
그 그룹을 한글 이름으로 지목**한다(예: "특히 사막 클래스의 샘플 품질이 41%로 낮음", "소득 컬럼의 유효성이 62%").
전 그룹이 비슷하면 굳이 지목하지 말 것. 없는 그룹·수치를 지어내지 말 것.
...
#### ❌ 레이블 일관성 (30%)
- 현황: 한 문장. groupBreakdown이 있고 특정 그룹/컬럼이 두드러지게 낮으면 그 이름을 지목(예: "특히 사막 클래스가 41%").
